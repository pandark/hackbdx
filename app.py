...
def get_eb_cost(id):
    ...
    if (res.status_code == 200):
        for key in res.json()['ticket_classes']:
            if ('cost' in key):
                ret = dict( display = key['cost']['display'], value = key['cost']['value'])
            else:
                ret = dict( display = 'Free', value = 0)
    return (ret);
...
def get_meet_price(key):
    ...
    if ('fee' in key):
        ret = dict(value = key['fee']['amount'] * 100, display = str(key['fee']['amount']) + ' ' + key['fee']['currency']);
    else:
        ret = dict(value = 0, display = 'Free');
    return (ret);
...
def main():
    ...
    sanitized = [v for v in array if lat in v['_geoloc'] and lng in v['_geoloc']]
